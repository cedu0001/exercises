...
//
//
function klik(evt) { // hiver værdien af data-func attributten ud af knapperne i HTML
  const btnClicked = evt.target.dataset.func; // skriver det der er klikket på til lommeregnerens skærm
  updateScreen(btnClicked); 
  typeOfInput = whatTypeOfInput(btnClicked);// hvilken salgs input er det: "num" "regneart" "ligmed" "CLEAR"
  ...
  if (typeOfInput === "num") {
    ...
    updateScreen(result); // den opdaterer skærmen med resultatet af regnestykket
    ...
    ((firstNumVal = ""), (secondNumVal =""), (regneArt = "")); // den nulstiller regnestykkets variabler
